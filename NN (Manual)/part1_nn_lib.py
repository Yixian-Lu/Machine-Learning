...
class SigmoidLayer(Layer):
    ...
    def forward(self, x):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        outputs = self.compute_sigmoid(x)
    ...
    def backward(self, grad_z):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        g_z = self.compute_sigmoid(self._cache_current)
        derivative = g_z * (1 - g_z) 
...
class ReluLayer(Layer):
    ...
    def backward(self, grad_z):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        derivative = self._cache_current
...
class LinearLayer(Layer):
    ...
    def backward(self, grad_z):
        ...
        one_vector = np.ones((grad_z.shape[0]))

        self._grad_W_current = x.T.dot(grad_z)
        self._grad_b_current = one_vector.T.dot(grad_z)
...
class Trainer(object):
    ...
    def train(self, input_dataset, target_dataset):
        ...
        for _ in range(self.nb_epoch):
            ...
            for i in range(0, N, self.batch_size):
                batch = input_dataset[i : i + self.batch_size]
                y_target = target_dataset[i : i + self.batch_size]
                y_pred = self.network.forward(batch)

                self._loss_layer.forward(y_pred, y_target)
    ...
    def eval_loss(self, input_dataset, target_dataset):
        ...
        y_target = target_dataset
        return self._loss_layer.forward(y_pred, y_target)
        #######################################################################
        #                       ** END OF YOUR CODE **
        #######################################################################
...
class Preprocessor(object):
    ...
    def __init__(self, data):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        self.minData = data.min(axis=0)
        self.maxData = data.max(axis=0)
        #######################################################################
        #                       ** END OF YOUR CODE **
        #######################################################################
    ...
    def apply(self, data):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        normalized_dataset = np.zeros([0,data.shape[1]])
        for i in range(data.shape[0]):
            row = []
            for j in range(data.shape[1]):
                numerator = data[i,j] - self.minData[j]
                denominator = self.maxData[j] - self.minData[j]
                normalized_data = numerator/denominator
                row.append(normalized_data)
            normalized_dataset = np.concatenate((normalized_dataset,[row]),axis=0)
    ...
    def revert(self, data):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        reversed_dataset =np.zeros([0,data.shape[1]])
        for i in range(data.shape[0]):
            row = []
            for j in range(data.shape[1]):
                reversed_factor = self.maxData[j] - self.minData[j]
                normalized_data = reversed_factor*data[i,j]+self.minData[j]
                row.append(normalized_data)
            reversed_dataset = np.concatenate((reversed_dataset,[row]),axis = 0)
